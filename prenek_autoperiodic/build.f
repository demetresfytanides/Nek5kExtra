...
c-----------------------------------------------------------------------
      subroutine tuple_swap_ip(x,m,n,p,start,t2) 

c     In-place permutation: x' = x(p)

      real x(m,n),xstart(m),t2(m)
      ...
      do k=1,n
         ...
         if (p(k).gt.0) then   ! not swapped
            ...
            do j=k,n
               next    = p(last)
               if (next.lt.0) then
                  write(6,*) 'Hey! tuple_swap_ip problem.',j,k,n,next
                  call exit
               elseif (next.eq.loop_start) then
                  call copy(x(1,last),xstart,m)
                  p(last) = -p(last)
                  goto 10
               else
