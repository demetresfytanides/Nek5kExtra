...
c-----------------------------------------------------------------------
      subroutine getnormals(unx,uny,unz,w,area,e,f,x,y,z,ndim)
      real x(8,nelm),y(8,nelm),z(8,nelm)
...
c     write(6,*) 'IN GETNORMAL: ',f,e,ld,ndim
...
c     write(6,*) 'B4 vcross: ',f,e
      call vcross_normal(w,r2,u,v)
c     write(6,*) 'AF vcross: ',f,e,r2
      unx = w(1)
      uny = w(2)
      unz = w(3)
      area = 2*r2
...
c
               call getnormals(unx,uny,unz,w,area,ie,is,x,y,z,nelm,ndim)
...
c                 goto 113
     ...
     $               ' of element ',edup,'$'
                  call prs(s)
                  call prs('is internal side A.  I won''t duplicate.$')
                  write(6,*) 'CBC3: ',cbc3
                  goto 900
c                 goto 113
     ...
     $               ' of element ',edup,'$'
                  call prs(s)
                  call prs('is Periodic side B.  I won''t duplicate.$')
                  write(6,*) 'CBC3: ',cbc3
                  stop
                  goto 900
c                 goto 113
     ...
     $               ' of element ',edup,'$'
                  call prs(s)
                  call prs('is Internal side C.  I won''t duplicate.$')
                  write(6,*) 'CBC3: ',cbc3
                  goto 900
c                 goto 113
               endif
...
C           Automatic selection of Periodic bc
            call fndsidb(js,je,is,ie,kf,tol)
            if (js.ne.0) then

               call letbc(js,je,kf,bclab)
               call letbc(is,ie,kf,bclab)

               call rzero(bc(1,is,ie,kf),5)
               call rzero(bc(1,js,je,kf),5)

               cbc (is,ie,kf) = 'P  '
               ibc (is,ie,kf) = je       ! high-precision periodic bc
               bc(1,is,ie,kf) = je
               bc(2,is,ie,kf) = js

               cbc (js,je,kf) = 'P  '
               ibc (js,je,kf) = ie       ! high-precision periodic bc
               bc(1,js,je,kf) = ie
               bc(2,js,je,kf) = is


               if (nel.le.1000.or.mod(ie,100).eq.0)
     $         write(6,1) kf,ie,is,je,js
...
c-----------------------------------------------------------------------
      subroutine score_pair(lf,score,x3k,unk,x3j,unj,ndim,jf)

      real x3k(3),x3j(3),unk(4),unj(4)
      real dx3(3),ux3(3)

      tol = 1.e-4

      if (abs(unk(3)).gt.0.01) return

      if (abs(unk(1)).gt.0.99.or.abs(unk(2)).gt.0.99) goto 9
      return
    9 continue

      unorm=0
      do j=1,ndim
         unorm = unorm + (unk(j)+unj(j))**2
      enddo
      unorm = sqrt(unorm)
c     if (unorm.gt.tol) return
      ...
      do j=1,ndim
         dxt = dx3(j)/dnorm
         wnorm  = wnorm + (unk(j)+dxt)**2
c        write(6,*) unk(j),dxt,j,' Unit Normal'
      enddo
c     if (wnorm.gt.tol) return

c     check scores

      exponent = 1./(ndim-1)
      char_length = unk(ndim+1)**exponent

      eps = 1.e-7*char_length

      projected_length = 0 
      do j=1,ndim
         projected_length = projected_length - unk(j)*dx3(j)
      enddo
      projected_length  = sqrt(projected_length)
      orthogonal_length = dnorm**2 - projected_length**2
      orthogonal_length = sqrt(orthogonal_length)
...
c                        .....       | h
c                 .....              |             l
c                O-------------------p     Large _____  is good.
c                         l                      h+eps

      score_t = projected_length / (eps+orthogonal_length)

c     write(6,*) unorm,wnorm,score_t,jf,' UNORM'

      if (unorm.gt.tol) return
      if (wnorm.gt.tol) return
      
      if (score_t.gt.score) then
         lf = jf
         score = score_t
      endif
      return
...
c-----------------------------------------------------------------------
      ...
      write(6,*) 'Paused (<cr> to continue).'
      read (5,*) 

      return
      end
c-----------------------------------------------------------------------
      subroutine get_x3(xx,e,f)
#     include "basics.inc"
...
c-----------------------------------------------------------------------
      subroutine get_unx_area(un,e,f)
#     include "basics.inc"
      ...
      call getnormals(unx,uny,unz,un,area,e,f,x,y,z,nelm,ndim)
      un(4) = area

      return
      end
c-----------------------------------------------------------------------
      integer function ibracket(x,xa,n,lohi)

      real xa(n)

c     find j such that xa(j)   < x <= xa(j+1) if lohi = 0
c     find j such that xa(j-1) < x <= xa(j)   if lohi = 1


c     Check total range   (assumption is that xa is sorted)
...
#     include "basics.inc"
      ...
      parameter(npmax=nelm*6)

      common /periodic/ un4(4,npmax),dotp(npmax)
      common /periodii/ lst(npmax+1),ind(npmax)

      character*3 cbu

      real t1(4),t2(4),tvec(4),x3k(3),x3j(3)

      integer e,f,ef,efo,fld,ej,fj,el,fl
      integer efl,efj
      ...
      do f=1,2*ndim
         ef = ef+1
c        if (cbc(f,e,fld).ne.'E  ') then
         if (cbc(f,e,fld).eq.'   ') then
            kf=kf+1
            lst(kf)=ef
            if (kf.gt.npmax) then
               write(6,*) 
     $         'TOO MANY PERIODIC BCS. INCREASE npmax IN bound.f'
               return
            endif
         endif
      enddo
      ...
      do kf=1,nf
         ef=lst(kf)
         call get_ef(e,f,ef,ndim)
         call get_unx_area(un4(1,kf),e,f)
      enddo

      tvec(1)=1.
      tvec(2)=1.7
      tvec(3)=1.95
      if (ndim.eq.2) tvec(3)=0
      ...
      do itrial = 1,10
         ...
         do kf=1,nf
            ...
            area=un4(4,kf)
            dotp(kf)=area*
     $        (tvec(1)*un4(1,kf)+tvec(2)*un4(2,kf)+tvec(3)*un4(3,kf))
            dotp_min = min(dotp_min,abs(dotp(kf)))
            area_min = min(area_min,area)
         enddo
         if (dotp_min.lt.0.001*area_min) then
            tvec(1) = 1.1*tvec(1)
            tvec(3) = 1.2*tvec(3)
         else
            goto 10  ! Found a sort key
         endif
      enddo
   10 continue

      call sort          (dotp,ind,nf)
      call swap_ip       (dotp,ind,nf)
      call iswap_ip      (lst,ind,nf)
      call tuple_swap_ip (un4,4,nf,ind,t1,t2) ! Tuple-swap

      do kf=1,nf

        ef = lst(kf)
        call get_ef (e,f,ef,ndim)
        call get_x3 (x3k,e,f) ! Get face centroid

        cbu = cbc(f,e,1)
c       write(6,*) 'IN KF LOOPX: ',kf,e,f,x3k(1),x3k(2),x3k(3),cbu
c       write(6,*) 'IN KF LOOPN: ',ef,e,f,un4(1,kf),un4(2,kf),un4(3,kf)
c       call pause
        ...
        if (dotf.gt.0) then
          jlo=ibracket(dotf09,dotp,nf,0)
          jhi=ibracket(dotf11,dotp,nf,1)
        else
          jlo=ibracket(dotf11,dotp,nf,0)
          jhi=ibracket(dotf09,dotp,nf,1)
        endif

c       write(6 ,*) kf,jlo,jhi,dotp(kf),dotp(jlo),dotp(jhi),' JLOHI'

        sc = 0
        do jf=jlo,jhi ! Explore candidate list
          area_diff = abs(un4(4,kf)-un4(4,jf))/un4(4,kf)


          tol_area = 1.e-4                ! Relative area tolerance
          if (area_diff.lt.tol_area) then ! Possible candidate

             efj = lst(jf)
             call get_ef (ej,fj,efj,ndim)
             call get_x3 (x3j,ej,fj) ! Get face centroid

             call score_pair(lf,sc,x3k,un4(1,kf),x3j,un4(1,jf),ndim,jf)
c            write(6 ,*) kf,jf,area_diff,un4(4,kf),un4(4,jf),sc,' AREA'
        ...
        enddo
          ...
          if (sc.gt.0) then
             efl = lst(lf)
             call get_ef       (el,fl,efl,ndim) 
             call set_match_p  (e,f,el,fl)
c            write(26,*) e,f,el,fl,' MATCH'
c            write(6 ,*) e,f,el,fl,' MATCH'
          endif
